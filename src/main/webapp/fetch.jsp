...
<%
    ...
    try {
        ...
        conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/expense", "root", "Saurabh@19091");
        String sql = "SELECT * FROM task_add WHERE uid = ?";
        ...
        <table class="table table-striped">
            ...
            <tbody>
                ...
                <tr id="row_<%= rs.getInt("task_id") %>">
                    ...
                    <td><button class='delete-btn' onclick='deleteRow(<%= rs.getInt("task_id") %>);'>Delete</button></td>
                    <td><button class='update-btn' onclick='updateRow(<%= rs.getInt("task_id") %>);'>Update</button></td> <!-- Update Button -->
                </tr>
                <% } %>
            </tbody>
        </table>
